...
function install_core_packages{
    ...
    # core install (some packages might be already installed)
     ...
     brew install zsh             # best shell to use 
    ...
    # basic tools to start with
     ...
     brew install guake           # ? drop down terminal emulator - # ! use x (x11 or xarg) display server since keybinding doesn't work well in wyaland server
    ...
    # system utilities
     ...
     brew install fd-find         # ? fdfind: a program to find entries in your filesytem. It is a simple, fast and user-friendly alternative to find
     ...
     brew install sysstat         # ? iostat - cpu usage
     brew install bridge-utils    # ? brctr - ethernet brdige admin cmd
    ...
    # just for fun
     ...
     brew install lolcat            # [Not in venv setup]
}
